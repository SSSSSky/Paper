...
def run_sparse_auto_encoder(n_input=16, n_hidden_1=5, batch_size=2048, transfer=tf.nn.sigmoid, epoches=500, rho=0.5,
    ...
    with tf.Session(config=config) as sess:
        ...
        # 获得隐藏层的数据，即所需要的信息
        train_hidden_result = sess.run(model_one_hidden, feed_dict={model_one_X: wt_train_df})
        valid_hidden_result = sess.run(model_one_hidden, feed_dict={model_one_X: wt_valid_df})
        test_hidden_result = sess.run(model_one_hidden, feed_dict={model_one_X: wt_test_df})
...
def main():
    run_sparse_auto_encoder(n_input=22, n_hidden_1=10, epoches=300, batch_size=2048, rho=0.1, beta=1.0, alpha=1e-4,
    ...
    # run_sparse_auto_encoder(n_input=47, n_hidden_1=10, epoches=300, batch_size=2048, rho=0.1, beta=1.0, alpha=1e-4, lamda=1.0, transfer=tf.nn.sigmoid, decay=1.0,
